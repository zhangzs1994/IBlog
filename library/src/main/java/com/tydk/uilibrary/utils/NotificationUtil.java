...
//                .build();
//        channel.setSound(Uri.parse("android.resource://" + context.getPackageName() + "/" + R.raw.cnwav),att);
    ...
    public static void show(Context context, String contentTitle, String contentText, RemoteViews views, int notifyId, String channelId, Class<?> cls) {
        show(context, 0, 0, contentTitle, null, contentText, NotificationManager.IMPORTANCE_HIGH, null, views, notifyId, channelId, cls);
    }
     ...
     * @param cls          意图类
     */
    public static void show(Context context, int icon, String contentTitle, String contentText, Class<?> cls) {
        show(context, icon, icon, contentTitle, null, contentText, NotificationManager.IMPORTANCE_HIGH, null, null, 0, NEW_MESSAGE, cls);
    }
    ...
    public static void show(Context context, int largeIcon,
        ...
        builder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), largeIcon == 0 ? R.mipmap.ic_launcher : largeIcon)) //设置自动收报机和通知中显示的大图标。
                ...
                //.setDefaults(Notification.DEFAULT_SOUND | Notification.DEFAULT_VIBRATE)// 设置默认的提示音、振动方式、灯光等 使用的默认通知选项
